...
def validate_required_fields(doc):
	fields_to_check = [
		"card_no", "authorization_number", "patient_name", "gender", "practitioner",
		"referrer_facility_code", "referring_diagnosis", "reason_for_referral",
	]

	for fieldname in fields_to_check:
		if not doc.get(fieldname):
			frappe.throw("Missing Value for field: {0}".format(frappe.bold(
