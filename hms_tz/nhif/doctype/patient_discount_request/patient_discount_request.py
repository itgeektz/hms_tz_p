                ...
                for item in self.items:
                    for row in si_doc.items:
                        if (
                            item.si_detail
                            == row.name
                            # and item.item_code == row.item_code
                        ):
                            if flt(self.discount_percent) > 0:
                                row.price_list_rate = 10000
                                row.rate = item.amount_after_discount
                                row.discount_percentage = self.discount_percent
                                row.discount_amount = item.discount_amount

                            if flt(self.discount_amount) > 0:
                                row.rate = item.amount_after_discount
                                # row.discount_percentage = 0
                                row.discount_amount = self.discount_amount

                            print("\n\n\n")
                            print("row.discount_amount", row.discount_amount)
                            print("\n\n\n")

            si_doc.hms_tz_discount_status = "Approved"
            si_doc.calculate_taxes_and_totals()
            for row in si_doc.payments:
                row.amount = 0
...
def get_patient_discount_request(data):
    ...
    pdr.discount_reason = data.get("discount_reason")
