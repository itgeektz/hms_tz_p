...
def get_columns():
    return [
        ...
        {
            "fieldname": "regency_insurance_claim",
            "label": _("Regency Insurance Claim"),
            "fieldtype": "Data",
        },
        {
            "fieldname": "signature",
...
def get_appointment_data(filters):
    ...
    npc = DocType("Regency Insurance Claim")
    ...
    q = (
        ...
        .where(
            ...
            & (pa.insurance_company.notin(["NHIF","NHIF Town","NHIF Upanga - RSPDC"]))
            & (pa.mode_of_payment == "")
            & (status_map)
        )
    ...
    if filters.patient_type == "In-patient":
        q = q.inner_join(ip).on(
            (pa.name == ip.patient_appointment) & (ip.insurance_company.notin(["NHIF","NHIF Town","NHIF Upanga - RSPDC"]))
        )
        q = q.select(
    ...
    if filters.patient_type == "Out-patient":
        inpatient_appointmens = (
            ...
            .where(
                (ip.insurance_company.notin(["NHIF","NHIF Town","NHIF Upanga - RSPDC"]))
