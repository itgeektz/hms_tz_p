            ...
            for item in sales_invoice_doc.items:
                if item.reference_dt == "Drug Prescription":
                    ...
                    for original_item in doc.hms_tz_original_items:
                        if (
                            original_item.hms_tz_is_out_of_stock == 1 and
                            item.name == original_item.si_detail and 
                            item.item_code == original_item.item_code and 
                            item.parent == original_item.against_sales_invoice
                        ):
                            frappe.db.set_value("Drug Prescription", item.reference_dn, {
                ...
                for original_item in doc.hms_tz_original_items:
                    if (
                        ...
                        for drug_item in patient_encounter_doc.drug_prescription:
                            if (
                                original_item.reference_name == drug_item.name and 
                                original_item.reference_doctype == drug_item.doctype
                            ):
