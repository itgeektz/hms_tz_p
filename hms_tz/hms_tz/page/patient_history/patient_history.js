...
frappe.pages['patient_history'].on_page_show = function (wrapper) {
	var patient = $('[data-fieldname="patient"]');
	if (frappe.route_options && frappe.route_options.patient) {
		patient.val(frappe.route_options.patient);
		wrapper.start = 0;
		wrapper.page.main.find(".patient_documents_list").html("");
		get_documents(frappe.route_options.patient, wrapper);
		show_patient_info(frappe.route_options.patient, wrapper);
		show_patient_vital_charts(frappe.route_options.patient, wrapper, "bp", "mmHg", "Blood Pressure");
	}
};