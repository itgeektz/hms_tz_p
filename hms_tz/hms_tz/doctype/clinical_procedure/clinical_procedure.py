...
class ClinicalProcedure(Document):
    ...
    def after_insert(self):
        if self.prescription:
            frappe.db.set_value(
                "Procedure Prescription", self.prescription, "procedure_created", 1
            )
            frappe.db.set_value(
        ...
        if template.sample:
            ...
            frappe.db.set_value(
                "Clinical Procedure", self.name, "sample", sample_collection.name
            )
                ...
                for item in self.items:
                    if item.invoice_separately_as_consumables:
                        ...
                        if not consumption_details:
                            consumption_details = _("Clinical Procedure ({0}):").format(
                                self.name
                            )
                        consumption_details += "\n\t" + item_consumption_details
...
def delete_pre_post_documents(doc):
    if doc.nursing_tasks:
        for nursing_task in frappe.get_list(
            "Healthcare Nursing Task",
...
def get_procedure_consumables(procedure_template):
    return get_items(
        "Clinical Procedure Item", procedure_template, "Clinical Procedure Template"
    )
...
def set_stock_items(doc, stock_detail_parent, parenttype):
    ...
    for item in items:
        ...
        if parenttype == "Clinical Procedure Template":
            se_child.invoice_separately_as_consumables = (
...
def make_procedure(source_name, target_doc=None):
    def set_missing_values(source, target):
        ...
        if consume_stock:
            ...
            set_stock_items(
                target, source.procedure_template, "Clinical Procedure Template"
            )
...
def insert_clinical_procedure_to_medical_record(doc):
    subject = (
        frappe.bold(_("Clinical Procedure conducted: "))
        + cstr(doc.procedure_template)
        + "<br>"
    )
    if doc.practitioner:
