...
class LimitChangeRequest(Document):
    ...
    def after_insert(self):
        ...
        if self.is_non_nhif_patient and self.appointment_no:
            self.previous_cash_limit = frappe.get_value(
                "Patient Appointment", self.appointment_no, "daily_limit"
            )
    ...
    def validate(self):
        ...
        if (
            ...
            and self.company
            and self.company
            != frappe.get_value("Patient Appointment", self.appointment_no, "company")
        ):
    ...
    def before_submit(self):
        ...
        if flt(self.previous_cash_limit) and flt(self.cash_limit) <= flt(
            ...
            frappe.throw(
                "New Cash/Daily Limit must be greater than the previous cash limit"
            )
        ...
        if self.is_non_nhif_patient and self.insurance_company and self.appointment_no:
            ...
            frappe.msgprint(
                f"Daily Limit of: <b>{self.cash_limit}</b> was updated for Patient <b>{self.patient}</b>"
            )
