...
class TherapySession(Document):
    ...
    def validate_duplicate(self):
        ...
        if overlaps:
            ...
            frappe.throw(overlapping_details, title=_(
...
def invoice_therapy_session(source_name, target_doc=None):
    def set_missing_values(source, target):
        target.customer = frappe.db.get_value(
            'Patient', source.patient, 'customer')
...
def get_therapy_item(therapy, item):
    ...
    item.description = _('Therapy Session Charges: {0}').format(
        therapy.practitioner)
    item.income_account = get_income_account(
        therapy.practitioner, therapy.company)
...
def insert_session_medical_record(doc):
    ...
    if doc.therapy_plan:
        subject += frappe.bold(_('Therapy Plan: ')) + \
            cstr(doc.therapy_plan) + '<br>'
    if doc.practitioner:
        subject += frappe.bold(_('Healthcare Practitioner: ')
                               ) + doc.practitioner
    subject += frappe.bold(_('Total Counts Targeted: ')) + \
        cstr(doc.total_counts_targeted) + '<br>'
